...
# def validate_datasets_in_pipeline_config(
#     pipeline_config_file: str, pipeline: str = None, fix_catalog: bool = False
# ) -> bool:
#     """Validate that datasets used by nodes are defined.

#     Loads pipeline config using ConfigLoader and compares the set of
#     datasets defined in node config against the set of datasets
#     defined in the datasets key.

#     Prints names of datasets that are used in node config but were not
#     defined in the datasets key.

#     If fix_catalog is True, then adds missing datasets to a
#     {original_file_name}_fix.yml in the project directory.

#     Args:
#         pipeline_config_file: path of pipeline configuration file
#         pipeline: pipeline name to load config for
#         fix_catalog: flag to show if fix should be generated
...
#     """
#     cl = ConfigLoader(pipeline_config_file=pipeline_config_file, pipeline=pipeline)
